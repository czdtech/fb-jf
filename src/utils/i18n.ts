 ...
 * 获取支持的语言列表
 */
export const supportedLocales = ['en', 'zh', 'es', 'fr', 'de', 'ja', 'ko'];
 ...
 * 为英文游戏生成静态路径
 */
export async function generateEnglishGamePaths(): Promise<Array<{
  params: { slug: string };
  ...
  for (const game of allGames) {
    ...
    console.log(`[DEBUG] Processing game: ${gameId}, parts: ${JSON.stringify(parts)}`);
    ...
    if (parts.length === 2 && parts[0] === 'en') {
      ...
      console.log(`[DEBUG] Generated English path: /${slug}, gameSlug=${game.slug}`);
    }
  }
 ...
 * 为指定语言的游戏生成静态路径
 */
export async function generateLocalizedGamePaths(targetLocale: string): Promise<Array<{
  params: { slug: string };
  props: { game: CollectionEntry<'games'>; locale: string };
}>> {
  const allGames = await getCollection('games');
  ...
  for (const slug of allSlugs) {
    ...
    if (gameToUse) {
      ...
      console.log(`[DEBUG] Generated ${targetLocale} path: /${targetLocale}/${slug}, using content from: ${gameToUse.id}`);
    }
  }

  console.log(`[DEBUG] Total ${targetLocale} paths generated: ${paths.length}`);
  return paths;
}