...
describe('SEO元数据和hreflang测试', () => {
  ...
  describe('基础hreflang链接生成', () => {
    it('应该为所有语言生成正确的hreflang链接', () => {
      ...
      expect(englishLink!.url).toBe('https://www.playfiddlebops.com/test-page/');
      expect(englishLink!.label).toBe('English');
      
      // 检查中文链接
      const chineseLink = links.find(link => link.code === 'zh');
      expect(chineseLink).toBeDefined();
      expect(chineseLink!.url).toBe('https://www.playfiddlebops.com/zh/test-page/');
    ...
    it('应该正确处理不同的路径格式', () => {
      ...
      testCases.forEach(({ input, expected }) => {
        ...
        const englishLink = links.find(link => link.code === 'x-default');
        expect(englishLink!.url).toBe(`https://www.playfiddlebops.com${expected}`);
      });
    });

    it('应该支持自定义基础URL', () => {
      const customBaseUrl = 'https://custom-domain.com';
      const links = generateHreflangLinks(mockLanguages, '/page/', customBaseUrl);
  ...
  describe('游戏页面hreflang生成（智能检测）', () => {
    it('应该只为存在翻译的语言生成hreflang', async () => {
      // sprunki-retake有英文、中文、西班牙文翻译
      const links = await generateGameHreflangLinks(mockLanguages, 'sprunki-retake');
      
      expect(links).toHaveLength(3); // 只有3种语言有翻译
    ...
    it('只有英文和中文翻译的游戏应该生成2个hreflang', async () => {
      const links = await generateGameHreflangLinks(mockLanguages, 'incredibox');
    ...
    it('只有英文翻译的游戏应该只生成英文hreflang', async () => {
      const links = await generateGameHreflangLinks(mockLanguages, 'sprunki-phase-5');
      
      expect(links).toHaveLength(1);
      
      const englishLink = links[0];
      expect(englishLink.code).toBe('x-default');
      expect(englishLink.url).toBe('https://www.playfiddlebops.com/sprunki-phase-5/');
    });

    it('不存在的游戏应该返回空数组', async () => {
      const links = await generateGameHreflangLinks(mockLanguages, 'non-existent-game');
  ...
  describe('hreflang URL格式验证', () => {
    ...
    it('应该正确处理特殊字符的游戏slug', async () => {
      ...
      const specialGame = {
        ...
        body: '',
        collection: 'games',
        data: { title: 'Test Game', description: 'Test', image: '/test.png', category: 'test' },
        render: async () => ({ Content: () => '' })
      };
      
      (getCollection as jest.Mock).mockResolvedValueOnce([specialGame]);
      
      const links = await generateGameHreflangLinks(mockLanguages, 'game-with-dashes-123');
      
      expect(links).toHaveLength(1);
      expect(links[0].url).toBe('https://www.playfiddlebops.com/game-with-dashes-123/');
    });
  });
...
describe('SEO标签和元数据验证', () => {
  ...
  describe('国际化标准合规性', () => {
    it('语言代码应该符合HTML lang属性标准', () => {
      ...
      links.forEach(link => {
        expect(expectedMappings[link.code as keyof typeof expectedMappings]).toBe(link.label);
      });
    });

    it('应该提供正确的双向语言映射', async () => {
      const gameLinks = await generateGameHreflangLinks(mockLanguages, 'sprunki-retake');
...
describe('性能和缓存优化', () => {
  ...
  describe('重复调用优化', () => {
    it('相同参数的重复调用应该保持一致', async () => {
      const result1 = await generateGameHreflangLinks(mockLanguages, 'sprunki-retake');
      const result2 = await generateGameHreflangLinks(mockLanguages, 'sprunki-retake');
