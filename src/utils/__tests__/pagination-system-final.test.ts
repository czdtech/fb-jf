...
describe("游戏ID格式验证", () => {
  ...
  it("应该拒绝无效的语言代码", () => {
    // 'invalid' 不是2字母语言代码，所以会被当作英文游戏ID
    // 只有正好2个字母的前缀才会触发语言验证
    const result = validateGameIdFormat("xx-fiddlebops.md"); // 使用不支持的2字母代码
...
describe("统一分页计算", () => {
  ...
  it("应该使用默认配置", () => {
    ...
    expect(result.totalPages).toBe(
      Math.ceil(100 / PAGINATION_CONFIG.GAMES_PER_PAGE),
    );
  });
});
...
describe("集成测试：完整分页流程", () => {
  it("应该完整处理分页请求", async () => {
    ...
    expect(lastPage.games.length).toBe(
      75 % PAGINATION_CONFIG.GAMES_PER_PAGE || PAGINATION_CONFIG.GAMES_PER_PAGE,
    );
  });

  it("应该正确处理分类筛选和分页", () => {
    const testGames = generateMockGames(50);
    const popularGames = testGames.filter(
      (game) => game.data.category === "popular",
    );

    const result = getPaginatedGames(popularGames, 1);

    expect(result.games.every((game) => game.data.category === "popular")).toBe(
      true,
    );
