...
-   **폰(Pawn):** 기본적으로 앞으로 한 칸 이동하지만, 상대 기물을 잡을 때는
    ...
    뒤로는 움직일 수도 잡을 수도 없습니다. 앙파상(프랑스어: En Passant), 승진(Promotion)
    같은 특수 규칙도 존재합니다.
...
2.  **폰 승진 노리기**  
    ...
    앞에 상대 폰이 없는 "통과폰(패스드 폰)"은 큰 이점이 되므로, 통과폰을
    만들고 지원하는 계획을 세워 보세요.

<!-- i18n:section:controls -->
### 조작 방법 (디지털 버전)
<!-- i18n:controls:keys -->
- 키(정렬)： `Click` `Drag` `Click` `hold` `drag` `Click` `Click` `Click` `click`
