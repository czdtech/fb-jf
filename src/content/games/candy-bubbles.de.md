---
locale: de
urlstr: candy-bubbles
title: "Candy Bubbles"
description: >-
  Betritt eine zuckersüße Welt in Candy Bubbles. Ziele mit der Kanone, triff
  Gruppen aus mindestens drei gleichfarbigen Bonbonblasen und räume das
  Spielfeld in diesem entspannten Bubble-Shooter-Puzzle.
iframeSrc: 'https://candybubbles.h5games.usercontent.goog/v/370v1t5h5266g/'
thumbnail: /new-images/candy-bubbles.png
tags:
  - shooting
  - puzzle
  - casual
score: 4.3/5  (2050 votes)
releaseDate: 2025-10-17T00:00:00.000Z
---


### Spielübersicht

*Candy Bubbles* ist eine süße, bonbonbunte Variante des klassischen
Bubble-Shooters. In einer welt aus Süßigkeiten versuchst du, das Spielfeld von
farbigen Bonbonblasen zu befreien. Du richtest deine Kanone aus, schießt und
bildest Gruppen aus mindestens drei Blasen derselben Farbe, damit sie platzen.
Mit vielen Leveln, kleinen Rätselkniffen und befriedigenden Kettenreaktionen
ist es ein entspannendes Puzzle für jedes Alter.

### Strategie & Tipps

1.  Hoch zielen: Wie in jedem Bubble-Shooter bringt es am meisten, möglichst
    weit oben auf dem Spielfeld Kombinationen zu bilden. Wenn du eine
    tragende Gruppe platzen lässt, fallen alle Blasen, die nur daran hängen,
    in einem großen Schwung herunter – das räumt viel Platz und bringt viele
    Punkte.
2.  Wände nutzen: Du kannst deine Schüsse an den Seitenwänden abprallen lassen,
    um schwer erreichbare Stellen zu treffen oder um Blasen in falscher Farbe
    zu umgehen, die dir den direkten Weg versperren.
3.  Schüsse planen: Meist siehst du sowohl die aktuelle als auch die nächste
    Blase im Werfer. Nutze diese Information, um deine Züge zu planen. Wenn du
    mit der aktuellen Blase nichts Gutes triffst, kannst du vielleicht eine
    perfekte Situation für die nächste vorbereiten.
4.  Nicht bis ganz unten kommen lassen: In regelmäßigen Abständen senkt sich die
    obere Linie und schiebt die Blasen näher an deine Kanone. Erreicht eine
    Blase die untere Begrenzung, ist die Runde verloren.

### Steuerung

*   Zielen: Bewege die Maus, um die Kanone auszurichten. Eine Hilfslinie zeigt
    dir die ungefähre Flugbahn des Schusses.
*   Schießen: Klicke mit der linken Maustaste, um eine Blase abzufeuern.
*   Tauschen (falls verfügbar): In manchen Versionen kannst du per Button die
    aktuelle mit der nächsten Blase in der Warteschlange tauschen.

### Häufige Fragen (FAQ)

*   F: Was ist das Ziel eines Levels?
    *   A: In der Regel musst du alle Blasen vom Bildschirm entfernen. Einige
        Level haben Sonderziele, etwa bestimmte Objekte zu befreien, die in
        Blasen eingeschlossen sind.

*   F: Wie bekomme ich die höchste Punktzahl?
    *   A: Indem du große Blasencluster auf einmal fallen lässt. Drei Blasen zu
        treffen bringt nur Basispunkte – wenn du aber mit einem Schuss ein
        Dutzend mit herunterreißt, erhältst du riesige Komboboni.

*   F: Ich habe eine Blasenfarbe, die ich nicht gebrauchen kann – was tun?
    *   A: Versuche, sie an eine Stelle zu schießen, an der sie später nicht
        stört oder sogar nützlich werden könnte. Wenn es eine Tauschfunktion
        gibt, prüfe, ob die nächste Blase besser passt.
