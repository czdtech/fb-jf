...
当任意一个当前给出的方块再也找不到可以容纳它的位置时，这一局就会结束。  
...
2.  **避免制造孤立单格空洞**  
    尤其要避免在中间区域留下单独一格的空洞，这几乎只能靠极少出现的单格方块来填，会严重浪费版面。尽量让整体轮廓保持相对平滑。
...
6.  **管理好“大方块”和长条**  
    这些大块是最容易导致猝死的形状。尽量在棋盘上保留一块足够大的连续空区，以及一条能容纳长条的直线空间。若你发现已经放不下这些形状，说明局面已经非常危险。
...
任务模式通常由多个关卡组成，每关都有各自的目标，比如在限定步数内消除一定行数、达到指定分数、或利用特殊方块完成指定动作。完成当前关卡目标即可解锁下一关。
