---
locale: fr
title: Google Snake
seoTitle: Jouez à Google Snake en ligne gratuitement | FiddleBops
description: >-
  Version moderne de Snake : guide un serpent qui grandit en mangeant des
  fruits, évite les collisions et vise les meilleurs scores avec différents
  modes de jeu.
iframeSrc: >-
  https://szhong.4399.com/4399swf//upload_swf/ftp44/chenling/20230830/05/index.html
thumbnail: /new-images/thumbnails/google-snake.jpg
urlstr: google-snake
tags:
  - arcade
  - snake
score: 3.8/5  (1205 votes)
releaseDate: 2025-08-16T00:00:00.000Z
---


<!-- i18n:section:introduction -->
### Présentation du jeu

Google Snake modernise le classique Snake avec des visuels propres et plusieurs variantes de règles. Mange des fruits pour grandir tout en évitant les murs et ton propre corps. La vitesse augmente au fil du temps, et certains modes modifient la taille du plateau, les obstacles ou les effets des objets.

<!-- i18n:section:how-to-play -->
### Comment jouer

- Planifie des trajectoires qui laissent toujours une voie de sortie ; ne t'enferme jamais toi‑même.
- Utilise l'anneau extérieur pour les longues lignes droites, puis replonge vers le centre pour récupérer les fruits.
- Adapte‑toi aux objets spéciaux et aux règles de mode activées par l'hébergeur.

<!-- i18n:section:controls -->
### Commandes
<!-- i18n:controls:keys -->
- Touches (alignement)： `Arrow keys` `WASD`

- Flèches directionnelles ou WASD pour diriger le serpent.
- Pause / restart via l'interface.

<!-- i18n:section:tips -->
### Astuces

- Ne fais pas de demi‑tour brutal à 180° sur ton propre corps — préfère les grands virages.
- Garde ta queue droite autant que possible pour maximiser les options.
- Quand la vitesse est élevée, anticipe tes virages une case à l'avance.

<!-- i18n:section:faq -->
### Foire aux questions (FAQ)

<!-- i18n:faq:id=faq:google-snake:how-can-i-avoid-running-bf744697 -->
- Q : Comment éviter de foncer dans ma propre queue ?

  R : Garde une queue compacte avec des segments droits et évite les spirales trop serrées.

<!-- i18n:faq:id=faq:google-snake:does-the-game-get-faster-e76218ed -->
- Q : Le jeu va‑t‑il plus vite avec le temps ?

  R : Oui, la plupart des versions augmentent la vitesse à mesure que ton score monte.

<!-- i18n:faq:id=faq:google-snake:are-there-different-board-setu-40b930d4 -->
- Q : Y a‑t‑il plusieurs types de plateaux ?

  R : Beaucoup d'hôtes proposent différentes tailles de grille, des obstacles et divers objets de bonus/malus.


