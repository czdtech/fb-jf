---
locale: de
urlstr: bank-robbery
title: "Bank Robbery"
description: >-
  Ein ausführlicher Guide zu Bank Robbery 2, einem teambasierten Ego‑Shooter
  rund um Banküberfälle. Lerne Strategien für Räuber und Cops, die Steuerung und
  Antworten auf häufige Fragen zum perfekten Raubzug.
iframeSrc: >-
  https://app-183971.games.s3.yandex.net/183971/l0eme2yqkthl0y8stmfuz0u8pdpux64m/index.html
thumbnail: /new-images/thumbnails/bank-robbery.jpg
tags:
  - fPS
  - action
score: 4.2/5  (1331 votes)
releaseDate: 2025-08-16T00:00:00.000Z
---


### Spielübersicht

*Bank Robbery 2* ist ein teambasierter Ego‑Shooter, in dem sich zwei Fraktionen
gegenüberstehen: **Räuber** und **Polizei**. Dreh‑ und Angelpunkt jeder Runde
ist ein Banküberfall.

Als **Räuber** infiltrierst du eine stark gesicherte Bank, setzt einen Bohrer
an Tresoren und Safes an, sammelst Geld und versuchst anschließend, mit der
Beute zum Fluchtpunkt zu entkommen – während dir die Polizei im Nacken sitzt.

Als **Cop** verteidigst du die Bank. Du musst verhindern, dass die Tresore
geknackt werden, laufende Bohrer entschärfen und das Räuberteam ausschalten,
bevor es mit dem Geld flieht. Beide Seiten nutzen Waffen, Ausrüstung und
teilweise Fahrzeuge, um ihre Ziele zu erreichen und Geld für bessere Ausrüstung
in zukünftigen Runden zu verdienen.

### Strategie

Der Spielstil hängt stark davon ab, auf welcher Seite du spielst.

#### Tipps für Räuber

1.  **Einstieg planen:** Stürmt nicht blind durch den Haupteingang. Nutzt
    Hinterhöfe, Seitentüren oder Dächer. Ein Angriff aus mehreren Richtungen
    überfordert die Verteidigung.
2.  **Rollen vergeben:** Legt fest, wer den Bohrer setzt und bewacht, wer den
    Haupteingang sichert und wer Flanken oder Fenster im Blick behält.
3.  **Bohrer schützen:** Der Bohrer ist laut und zieht Cops an. Baut
    Verteidigungspositionen um den Bohrpunkt und haltet diesen Raum unter
    Kontrolle, bis das Geld frei ist.
4.  **Escape separat planen:** Der Überfall endet nicht mit dem Öffnen des
    Tresors. Sobald ihr Geldtaschen tragt, seid ihr extrem sichtbar. Legt im
    Voraus fest, über welche Route ihr zum Fluchtfahrzeug wollt und ob jemand
    als Fahrer bereitsteht.
5.  **Ausrüstung verbessern:** Nutzt verdientes Geld, um bessere Waffen,
    Panzerung und Tools wie C4 zu kaufen, damit ihr in späteren Runden schneller
    und sicherer eindringen könnt.

#### Tipps für Cops

1.  **Alle Eingänge sichern:** Verteidigt nicht nur die Lobby. Teilt euch auf,
    um Seitentüren, Hintereingänge und mögliche Sprengpunkte im Auge zu
    behalten.
2.  **Kommunikation:** Meldet im Teamchat, wo ihr Räuber seht und wo Bohrer
    gestartet wurden. So kann das Team schnell auf die richtige Stelle reagieren.
3.  **Gemeinsam zurückerobern:** Wenn die Räuber einen Bereich kontrollieren,
    brecht nicht einzeln hinein. Startet koordinierte Gegenangriffe aus mehreren
    Richtungen.
4.  **Bohrer entschärfen:** Läuft ein Bohrer, hat dessen Deaktivierung oberste
    Priorität. Schafft euch Deckung und schützt den Spieler, der entschärft, da
    dieser währenddessen wehrlos ist.
5.  **Fluchtwege abschneiden:** Haben die Räuber Geld erbeutet, verlagert sich
    euer Fokus auf Verfolgung und Blockade. Nutzt Polizeiwagen, um sie zu
    verfolgen, oder richtet Sperren auf den erwarteten Routen ein.

### Steuerung

Die Steuerung orientiert sich an gängigen PC‑Ego‑Shootern (z. B. Roblox).

- **`W`, `A`, `S`, `D`:** Bewegen
- **Maus:** Zielen und Kamera
- **Linke Maustaste:** Schießen
- **Rechte Maustaste:** Zielen (ADS)
- **`R`:** Nachladen
- **`Left‑Shift`:** Sprint
- **`Leertaste`:** Springen
- **`E` / `F`:** Mit Objekten interagieren (Bohrer setzen/entschärfen, Geld
  aufheben, Fahrzeuge betreten)
- **Zifferntasten (`1`, `2`, `3`, …):** Waffen/Items wechseln
- **`C`:** Ducken (falls in der jeweiligen Version vorhanden)

### Häufige Fragen (FAQ)

- **F: Wie komme ich an bessere Waffen und Ausrüstung?**  
  **A:** Durch gewonnene Runden und erfüllte Ziele erhältst du In‑Game‑Geld, das
  du im Shop gegen stärkere Waffen, bessere Rüstungen und Tools eintauschen
  kannst.

- **F: Unser Bohrer wird ständig entschärft – was tun?**  
  **A:** Lasst den Bohrer nie unbewacht. Mindestens ein bis zwei Teammitglieder
  sollten den Raum sichern, während der Rest Zugänge blockiert oder die Polizei
  ablenkt.

- **F: Wir werden als Cops dauernd überrannt – wie verteidigen wir besser?**  
  **A:** Verteidigt die gesamte Bank statt nur einen Punkt. Frühzeitige Info,
  von wo der Angriff kommt, erlaubt euch Gegenangriffe aus dem Rücken der
  Räuber.

- **F: Gibt es den perfekten Weg in die Bank für Räuber?**  
  **A:** Einen allgemein besten Weg gibt es nicht – alles hängt von der
  Aufstellung der Cops ab. Überraschende Seiteneingänge oder Sprengungen von
  Seitenwänden sind oft erfolgreicher als der offene Weg durch die Front.

- **F: Kann ich während einer laufenden Runde das Team wechseln?**  
  **A:** In der Regel nicht. Die Teamwahl erfolgt vor Rundenbeginn und bleibt
  bis zum Ende der Runde bestehen, um das Matchmaking fair zu halten.