...
2.  **Select a Color:** Tap on a color from the palette at the bottom of the screen. The game will highlight all the pixels on the grid that correspond to that color's number.
3.  **Fill the Pixels:** Zoom in on the highlighted areas and tap each numbered pixel to fill it with the selected color. Many games allow you to click and drag to quickly fill in adjacent pixels of the same number.
4.  **Complete the Picture:** Continue this process for every color in the palette. Once a color is fully filled in, it will often be grayed out or marked with a check, indicating you are done with that number. The puzzle is complete when every pixel has been colored.
