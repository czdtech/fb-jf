---
locale: ko
urlstr: ducklings
title: Ducklings 덕클링스
description: '호수 위를 헤엄치며 길 잃은 새끼 오리들을 구해 오는 힐링 게임입니다. 장애물을 피해 새끼들을 이끌고, 둥지를 업그레이드해 보세요.'
iframeSrc: 'https://g2.igroutka.ru/games/164/BkDlKfiWjEoZ2rcG/ducklings_io/'
thumbnail: /new-images/thumbnails/ducklings.jpg
tags:
  - animal
  - casual
score: 4.0/5  (1129 votes)
releaseDate: 2025-08-16T00:00:00.000Z
---


<!-- i18n:section:introduction -->
### 상세 게임 소개

Ducklings에서는 엄마 오리가 되어 호수 위를 돌아다니며, 물 위에 떠다니는 새끼
오리들을 찾아 둥지로 데려와야 합니다. 주변에는 보트를 비롯한 각종 장애물이
떠다니고 있어, 부딪히지 않도록 조심하면서 새끼들을 최대한 많이 안전하게
귀환시키는 것이 목표입니다. 모은 새끼 수에 따라 둥지의 크기와 장식이 점점
업그레이드됩니다.

<!-- i18n:section:how-to-play -->
### 플레이 방법

- 화면을 드래그하거나 방향키를 사용해 오리를 움직여, 호수 곳곳에 흩어진 새끼
  오리들에게 다가가면 자동으로 뒤를 따라오게 됩니다.  
- 보트나 장애물에 부딪히면 뒤따라오던 새끼들이 흩어질 수 있으니, 회피 경로를
  미리 생각하며 이동하세요.  
- 일정 수의 새끼를 둥지까지 무사히 데려오면 둥지가 업그레이드되고, 더 많은
  장식과 오리들이 등장합니다.  

<!-- i18n:section:controls -->
### 조작 방법
<!-- i18n:controls:keys -->
- 키(정렬)： `Mouse`

- 마우스/터치 드래그 또는 방향키로 오리를 이동시킵니다.  

<!-- i18n:section:tips -->
### 팁

- 호수 가장자리만 돌기보다, 맵 전체를 넓게 돌며 새끼 오리가 많이 모이는
  구역을 빠르게 찾아보세요.  
- 장애물이 많은 구역에서는 속도를 줄이고, 큰 원을 그리듯 돌아 위험을 피하는 것이 좋습니다.
- 무리가 너무 길어지면 뒤쪽이 장애물에 걸리기 쉬우니, 줄을 짧고 촘촘하게 유지하세요.

<!-- i18n:section:faq -->
### 자주 묻는 질문(FAQ)

<!-- i18n:faq:id=faq:ducklings:is-there-a-time-limit-31247f0f -->
- Q: 시간 제한이 있나요?  
  A: 보통은 없습니다. 급하게 하기보다 안전하게 데려오는 것이 중요합니다.
<!-- i18n:faq:id=faq:ducklings:what-ends-a-run-ed280e19 -->
- Q: 한 판이 끝나는 조건이 있나요?  
  A: 충돌로 뒤따라오던 새끼 오리를 잃으면 수확이 줄어듭니다. 둥지는 안전 지대이므로
  무리를 정리하고 다시 출발하면 됩니다.
<!-- i18n:faq:id=faq:ducklings:are-there-powerups-0fea9ab4 -->
- Q: 파워업이 있나요?  
  A: 호스트/버전에 따라 부스트나 코스메틱이 추가되기도 하지만, 기본 루프는 구조→귀환입니다.


