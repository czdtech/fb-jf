...
1.  **Create Festive Power-ups:** The best way to clear the board is by making special items from larger matches.
    *   **Match 4:** Creates a special item that can clear an entire row or column.
    *   **Match 5 (L or T shape):** Creates a wrapped candy bomb that explodes and clears a large area.
    *   **Match 5 (in a line):** Creates a powerful color bomb that can clear all items of a single color from the board.

2.  **Combine for Holiday Cheer:** Swap two special items with each other to unleash a massive, board-clearing effect. This is the key to solving the most challenging puzzles.
