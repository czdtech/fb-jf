---
locale: fr
urlstr: wood-block-match
title: Wood Block Match
description: >-
  Wood Block Match est un puzzle de placement de blocs relaxant : place des
  formes en bois, efface des lignes et préserve l'espace avec une stratégie
  maligne.
iframeSrc: 'https://2rfr65hktjv5o.h5games.usercontent.goog/v/46h85f5g0sldo/'
thumbnail: /new-images/thumbnails/wood-block-match.png
tags:
  - puzzle
  - thinky
  - casual
score: 4.1/5  (1969 votes)
releaseDate: 2025-10-18T00:00:00.000Z
---


<!-- i18n:section:introduction -->
### Présentation du jeu

Wood Block Match est un jeu de placement de blocs style 10×10 avec une esthétique bois. À chaque tour, tu reçois un lot de formes ; place-les pour compléter des lignes/colonnes et libérer de l'espace. La partie se termine quand aucune des pièces restantes ne rentre — le succès réside donc dans la préservation de couloirs flexibles tout en préparant des effacements multi-lignes.

<!-- i18n:section:tips -->
### Guide de jeu

1.  **Espace central :** Garde le centre du plateau ouvert ; arrange les bords pour éviter les pièges à une seule case.
2.  **Planification de poches :** Réserve une poche 2×2 ou en L pour les triominos gênants.
3.  **Pièce difficile d'abord :** Quand c'est encombré, place d'abord la pièce la plus grande/gênante.
4.  **Double effacement :** Construis des configurations à deux lignes pour qu'une pièce en efface deux.
5.  **Discipline d'annulation :** Si disponible, annule immédiatement après un placement qui scelle des poches.

<!-- i18n:section:controls -->
### Commandes

-   Glisse les pièces dans la grille ; tourne si le mode le permet ; effacer des lignes/colonnes rapporte des points.

<!-- i18n:section:faq -->
### Foire aux questions (FAQ)

<!-- i18n:faq:id=faq:wood-block-match:i-run-out-of-space-d59ef83f -->
-   **Q : Je manque d'espace très vite — que changer ?**
    **R :** Protège le centre, prévois une poche pour les pièces gênantes, et privilégie les doubles effacements aux simples.

<!-- i18n:faq:id=faq:wood-block-match:can-i-rotate-pieces-cd6fa3b2 -->
-   **Q : Puis-je tourner les pièces ?**
    **R :** Certaines variantes interdisent la rotation — compense avec une meilleure gestion des poches.

<!-- i18n:faq:id=faq:wood-block-match:any-tip-for-high-scores-87d58841 -->
-   **Q : Un conseil pour les scores élevés ?**
    **R :** Enchaîne les doubles effacements consécutifs et évite les simples d'urgence qui cassent la structure.

