---
locale: es
urlstr: merge-melons
title: "Merge Melons - Suika Game de frutas y sandías gigantes"
description: >-
  Suelta y fusiona frutas en este adictivo puzle inspirado en Suika
  Game. Combina frutas idénticas para crear versiones más grandes, apunta a la
  sandía gigante y evita que el contenedor se desborde.
iframeSrc: 'https://mergemelons.h5games.usercontent.goog/v/7526h5to2d1v0/'
thumbnail: /new-images/thumbnails/merge-melons.png
tags:
  - puzzle
  - casual
  - thinky
score: 4.7/5  (2990 votes)
releaseDate: 2025-10-17T00:00:00.000Z
---



### Introducción detallada al juego

Merge Melons es un puzle de físicas delicioso y muy adictivo, conocido
popularmente como "Suika Game" o "Watermelon Game". La premisa es sencilla pero
hipnótica: vas soltando distintos tipos de frutas dentro de un contenedor. Cuando
dos frutas idénticas se tocan, se fusionan y dan lugar a una fruta más grande
siguiente en el ciclo de evolución. Tu objetivo es crear la fruta más grande
posible —la sandía— y lograr la máxima puntuación sin que la pila llegue a la
parte superior del recipiente.

### Estrategia y recorrido de juego

1.  **Conocer el ciclo de frutas:** memoriza el orden de evolución. Saber qué
    fruta surge de combinar dos iguales (por ejemplo, dos uvas dan una naranja)
    es clave para planificar los lanzamientos.
2.  **Colocación estratégica:** no sueltes fruta al azar. Intenta situar las
    piezas pequeñas donde probablemente se encuentren con sus gemelas. Evita
    dejar caer una fruta muy grande encima de una mucho más pequeña, porque
    puede atraparla y desperdiciar espacio útil.
3.  **Preparar reacciones en cadena:** lo más satisfactorio del juego son las
    cadenas. Intenta colocar frutas de forma que una fusión provoque que la
    nueva fruta ruede o caiga sobre otra pareja y así sucesivamente. Es la
    mejor forma de despejar espacio y disparar la puntuación.
4.  **Gestionar el espacio:** la partida termina cuando las frutas se apilan y
    cruzan la línea superior. Vigila siempre cuánto espacio te queda. A veces
    conviene hacer una fusión menos óptima solo para ganar aire.

### Guía de controles

Los controles son muy sencillos.

*   **Apuntar (PC):** mueve el ratón a izquierda y derecha para decidir desde
        dónde se soltará la fruta.
*   **Soltar la fruta (PC):** haz clic con el botón izquierdo para dejar caer la
        fruta.
*   **Controles móviles:** desliza el dedo a izquierda y derecha para apuntar y
        levántalo para soltar la fruta.

### Preguntas frecuentes (FAQ)

*   **P: ¿Cuál es el objetivo principal?**
    *   **R:** Conseguir la máxima puntuación posible fusionando frutas sin
        parar. El logro máximo es formar una sandía combinando dos frutas del
        segundo nivel más alto.

*   **P: ¿Cómo se pierde la partida?**
    *   **R:** Pierdes cuando la pila de frutas toca o supera la línea marcada
        en la parte superior del contenedor.

*   **P: ¿Puedo controlar hacia dónde se moverán las frutas tras fusionarse?**
    *   **R:** No directamente, porque el juego se basa en físicas. Pero, si
        eliges bien el punto de caída, puedes influir en cómo rodará la fruta y
        dónde terminará asentándose la pieza fusionada.
