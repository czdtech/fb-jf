---
locale: ko
urlstr: om-nom-run
title: 'Om Nom: Run 옴놈 런'
description: >-
  Om Nom: Run에서는 옴놈과 옴넬과 함께 거리를 질주하며 장애물을 피하고 코인을 모으고, 다양한 파워업을 활용해 얼마나 멀리까지 달릴
  수 있는지 도전할 수 있습니다.
iframeSrc: 'https://omnomrun.h5games.usercontent.goog/v/38udj70raubl0/'
thumbnail: /new-images/thumbnails/om-nom-run.png
tags:
  - action
  - casual
score: 4.5/5  (3400 votes)
releaseDate: 2025-10-17T00:00:00.000Z
---



### 게임 상세 소개

*Om Nom: Run*은 "Cut the Rope" 시리즈의 귀여운 괴물 옴놈이 주인공인 러너 게임입니다.
옴놈이 도시의 거리와 지붕, 터널을 빠르게 달리며 장애물을 피하고 코인을 모아 미션을
완수해야 합니다. 달리는 동안 다양한 부스터를 활용해 더 멀리, 더 높은 점수에 도전할 수
있습니다.

### 공략 & 플레이 팁

1. **스와이프 숙달:** 기본 조작은 좌우 스와이프로 세 개의 레인 사이를 이동하고,
   위로 스와이프해 낮은 장애물을 점프, 아래로 스와이프해 높은 장애물 아래로 슬라이딩하는
   것입니다. 타이밍을 정확히 맞추는 연습이 중요합니다.
2. **파워업 적극 활용:** 달리다 보면 여러 특수 아이템이 등장합니다.
    - **자석:** 일정 시간 동안 주변의 코인을 자동으로 끌어모읍니다.
    - **로켓/제트팩:** 공중으로 떠올라 장애물을 모두 피하면서 코인을 수월하게 수집할 수
     있습니다.
    - **더블 코인:** 제한 시간 동안 획득하는 코인의 가치를 두 배로 올려 줍니다.
3. **미션 집중:** "점프 10번 하기", "코인 200개 모으기" 같은 미션을 달성하면 점수 배수와
   보상이 빠르게 증가하므로, 미션 조건을 염두에 두고 플레이하면 효율적으로 점수를
   올릴 수 있습니다.
4. **부스터 업그레이드:** 모은 코인으로 메인 메뉴에서 파워업 지속 시간과 효과를
   업그레이드하세요. 자석이나 로켓의 지속 시간이 늘어나면 스코어 상승 폭이 크게
   커집니다.

### 조작 방법


The game uses simple and intuitive swipe gestures.
* **좌우 스와이프:** 세 개의 레인 사이를 이동합니다.
* **위로 스와이프:** 장애물을 뛰어넘습니다.
* **아래로 스와이프:** 장애물 아래로 미끄러져 통과합니다.

### 자주 묻는 질문(FAQ)

* **Q: 게임의 주요 목표는 무엇인가요?**  
  **A:** 가능한 한 멀리 달려 최고 점수를 기록하는 것입니다. 달리는 동안 미션을 많이
  완료할수록 점수 배수가 올라가 더 높은 점수를 얻을 수 있습니다.

    - **Q: 장애물에 부딪히면 어떻게 되나요?**  
  **A:** 바로 런이 종료됩니다. 다만, 열쇠 등 특수 자원을 사용해 한 번 더 이어서 달릴
  수 있는 선택지가 제공되는 경우도 있습니다.

* **Q: Om Nelle 같은 다른 캐릭터는 어떻게 해금하나요?**  

    - **A:** New characters can usually be unlocked by collecting a large number of coins or by gathering special character tokens that appear randomly during runs.

- **Q: How do I unlock other characters like Om Nelle?**

    - **A:** Your run ends. However, you often have the option to spend a special currency (like keys) to get a second chance and continue your run from where you crashed.
  **A:** 보통 일정량의 코인을 모으거나, 러닝 중에 등장하는 캐릭터 토큰을 모으면
  새로운 캐릭터가 해금됩니다.
