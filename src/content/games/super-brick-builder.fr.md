---
locale: fr
urlstr: super-brick-builder
title: Super Brick Builder
description: >-
  Super Brick Builder est un puzzle d’empilement satisfaisant : pose des briques
  proprement, planifie les appuis et complète des structures sans effondrement.
iframeSrc: 'https://0hlb6nt2jsom8.h5games.usercontent.goog/v/4jac78qnv0nr8/'
thumbnail: /new-images/thumbnails/super-brick-builder.png
tags:
  - puzzle
  - thinky
  - casual
score: 4.0/5  (1873 votes)
releaseDate: 2025-10-18T00:00:00.000Z
---

<!-- i18n:section:introduction -->
### Présentation détaillée

Super Brick Builder se joue comme un puzzle d’empilement avec une physique : tu
places des briques pour compléter des silhouettes sans que la structure ne
s’effondre. Une pose propre et des appuis solides valent mieux qu’un remplissage
agressif.

<!-- i18n:section:tips -->
<!-- i18n:section:how-to-play -->
### Stratégies de jeu

1.  **Base d’abord :** construis une fondation stable et symétrique avant de
    monter.
2.  **Contrôle des porte‑à‑faux :** évite les grands débords sans contrepoids.
3.  **Ajuste avec les petites pièces :** garde des briques courtes pour combler
    les écarts et les zones de stress.
4.  **Rythme :** ralentis sur les couches supérieures pour éviter les petites
    poussées qui déstabilisent tout.
5.  **Recommencer :** des resets rapides aident à itérer tes designs.

<!-- i18n:section:controls -->
### Commandes
<!-- i18n:controls:keys -->
- Touches (alignement)： `Drag`

- Glisse / fais pivoter les briques ; relâche pour poser ; recommencer via
  l’interface.

<!-- i18n:section:faq -->
### Foire aux questions (FAQ)

<!-- i18n:faq:id=faq:super-brick-builder:structures-topple-62d97714 -->
- **Q : La structure s’effondre.**  
  **R :** Renforce la base et réduis les porte‑à‑faux.

<!-- i18n:faq:id=faq:super-brick-builder:run-out-of-fitting-pieces-581ced9c -->
- **Q : Je manque de pièces qui s’emboîtent.**  
  **R :** Garde des petites briques pour ajuster la couche du dessus.

