---
locale: en
title: "Kings and Queens"
description: "Kings and Queens is a regal match‑3 adventure — clear obstacles, unlock boosters, and restore royal halls with clever planning."
iframeSrc: "https://0ptbttt09g5cg.h5games.usercontent.goog/v/6c7l9okboa1u0/"
thumbnail: /new-images/thumbnails/kings-and-queens.png
urlstr: "kings-and-queens"
score: "4.0/5  (1892 votes)"
tags: ["puzzle", "casual", "thinky"]
developer: ""
releaseDate: 2025-10-18
---



<!-- i18n:section:introduction -->
### Detailed Game Introduction

Kings and Queens wraps a castle‑restoration story around classic match‑3. Each chapter introduces new blockers (chains, marble slabs, fog) and scene goals (collect crowns, break seals). You spend earned stars to renovate rooms, unlocking boosters for harder boards.

<!-- i18n:section:tips -->

<!-- i18n:section:how-to-play -->
### Gameplay Strategy & Walkthrough

1.  Board control: Clear central blockers first to open space for combos.
2.  Booster timing: Save hammers/rockets for late‑board choke points.
3.  Combo planning: Merge power‑ups (bomb+line/color clears) right before tight goals.
4.  Star economy: Spend on boosters that directly help your current hurdle.
5.  Move math: Count remaining blockers vs moves and play for the objective, not points.

<!-- i18n:section:controls -->
### Controls Guide

- `Drag` to swap; `tap` boosters to use; scene menu to renovate with stars.

<!-- i18n:section:faq -->
### Frequently Asked Questions (FAQ)

- <!-- i18n:faq:id=faq:kings-and-queens:im-stuck-on-a-marble-aed5c258 -->
  Q: I’m stuck on a marble slab level — any tip?
  
  A: Use bombs near slabs and chain line clears through them to crack multiple layers.

- <!-- i18n:faq:id=faq:kings-and-queens:should-i-hoard-boosters-c44e2837 -->
  Q: Should I hoard boosters?
  
  A: Keep a few, but spend to break brick‑wall stages; progression matters.

- <!-- i18n:faq:id=faq:kings-and-queens:renovations-affect-gameplay-b116edd0 -->
  Q: Renovations affect gameplay?
  
  A: Mostly cosmetic; some unlock booster slots or daily rewards.


