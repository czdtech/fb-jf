---
locale: ja
title: Solitaire 0-21
seoTitle: 無料でSolitaire 0-21をオンラインでプレイ | FiddleBops
description: Solitaire 0-21は、合計を0〜21に保ちながら進めるソリティアです。リスクと余裕のバランスを見てカードを配置します。
iframeSrc: 'https://4hcssrsmrpvv0.h5games.usercontent.goog/v/2oipnrim9ar08/'
thumbnail: /new-images/thumbnails/solitaire-0-21.png
urlstr: solitaire-0-21
tags:
  - card
  - puzzle
  - thinky
score: 4.0/5  (1885 votes)
developer: ''
releaseDate: 2025-10-18T00:00:00.000Z
---


<!-- i18n:section:introduction -->
### 詳細なゲーム紹介

Solitaire 0-21は、ソリティアに算術の制約を加えたゲームです。数字カードを配置して、進行中の合計を0〜21の範囲に保ちます。修飾子（+/-/×）や特殊効果が絡むため、先読みが重要です。安全圏を保ちつつ、急な増減に備えて余裕を残すのがポイントです。

<!-- i18n:section:tips -->
<!-- i18n:section:how-to-play -->
### ゲームプレイ戦略とウォークスルー

1.  バッファを残す：早い段階で合計を端に寄せず、余裕を持たせます。
2.  修飾子のタイミング：合計が安全な範囲に収まる見込みがあるときだけ乗算を使います。
3.  行の計画：リスクを行全体に分散し、1つの行に偏らせないようにします。
4.  高い数を保持：マイナスで相殺できる見込みが立つまで高いカードを温存します。
5.  アンドゥを活用：詰まりに気づいたら早めに戻して立て直します。

<!-- i18n:section:controls -->
### 操作ガイド
<!-- i18n:controls:keys -->
- キー（整合）： `Click` `drag`

- クリック/ドラッグしてカードを配置します。UIに従ってアンドゥ/ヒントを使用します（ある場合）。

<!-- i18n:section:faq -->
### よくある質問（FAQ）

<!-- i18n:faq:id=faq:solitaire-0-21:totals-keep-busting-a8b7e346 -->
- Q: 合計が破綻し続ける。
  
  A: 0/21の端に近すぎてプレイしている；クッションを残し、修飾子のペースを調整すること。

<!-- i18n:faq:id=faq:solitaire-0-21:best-way-to-use-negatives-0969db9d -->
- Q: ネガティブを使う最良の方法は？
  
  A: 高い数とペアにして、同じ行の余裕をリセットする。

<!-- i18n:faq:id=faq:solitaire-0-21:is-luck-dominant-bed2b357 -->
- Q: 運が支配的ですか？
  
  A: ドローは重要だが、配置戦略が結果に大きく影響する。


