---
title: Super Brick Builder
description: Super Brick Builderは、ブロックを壊すクラシックなブレイクアウトゲームです。パドルとボールを使ってすべてのブロックを破壊しましょう。
iframeSrc: 'https://0hlb6nt2jsom8.h5games.usercontent.goog/v/4jac78qnv0nr8/'
thumbnail: /new-images/thumbnails/super-brick-builder.png
urlstr: super-brick-builder
score: 4.0/5  (1873 votes)
tags:
  - arcade
  - breakout
  - classic
locale: ja
releaseDate: 2025-10-18T00:00:00.000Z
---

<!-- i18n:section:introduction -->
### ゲーム紹介

Super Brick Builderは、クラシックなブロック破壊ゲームの現代版です。パドルを動かしてボールを跳ね返し、画面上のすべてのブロックを破壊してレベルをクリアしましょう。

<!-- i18n:section:tips -->
### ゲームプレイガイド

 - **パドルを動かす：** パドルを左右に動かします
 - **ボールを跳ね返す：** ボールが落ちないようにパドルで跳ね返します
 - **ブロックを壊す：** ボールをブロックに当てて破壊します
 - **パワーアップ：** 特別なアイテムを集めて能力を強化します


- リトライ：素早くやり直すと、安定した手順を作りやすいです。
<!-- i18n:section:controls -->
### 操作ガイド

 - **マウス/矢印キー：** パドルを左右に移動

<!-- i18n:section:faq -->
### よくある質問（FAQ）


<!-- i18n:faq:id=faq:super-brick-builder:structures-topple-62d97714 -->
- Q: ボールがすぐ落ちます。
<!-- i18n:faq:id=faq:super-brick-builder:run-out-of-fitting-pieces-581ced9c -->
**ゲームの目標は何ですか？**


