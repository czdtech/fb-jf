---
locale: es
urlstr: letter-links
title: Letter Links
description: >-
  Letter Links es un desafío diario de construcción de palabras. Coloca tus
  fichas de letras en el tablero para formar palabras de alta puntuación,
  aprovecha las casillas de bonificación y conecta todas tus letras en un solo
  grupo.
iframeSrc: 'https://7u583tvdi3f00.h5games.usercontent.goog/v/424maigpuljoo/'
thumbnail: /new-images/thumbnails/letter-links.png
tags:
  - puzzle
  - thinky
  - casual
score: 4.6/5  (2850 votes)
releaseDate: 2025-10-17T00:00:00.000Z
---


<!-- i18n:section:introduction -->
### Introducción detallada al juego

Letter Links es un juego estratégico de crear palabras que recuerda a Scrabble.
Cada día recibes un nuevo conjunto de fichas de letras, cada una con un valor
en puntos. Tu objetivo es colocarlas en el tablero para formar palabras válidas
interconectadas, como en un crucigrama. Aprovechando letras de alto valor y las
casillas especiales de bonificación del tablero, intentas conseguir la mayor
puntuación posible.

<!-- i18n:section:tips -->
<!-- i18n:section:how-to-play -->
### Estrategia y recorrido de juego

1.  **Conoce tus bonificaciones:** La clave para puntuar alto es usar bien las
    casillas de bonificación.
    *   **DL (Double Letter):** Duplica el valor de la letra colocada ahí.
    *   **TL (Triple Letter):** Triplica el valor de la letra colocada ahí.
    *   **DW (Double Word):** Duplica la puntuación de toda la palabra que
        pase por esa casilla.
    *   **TW (Triple Word):** Triplica la puntuación de toda la palabra que
        pase por esa casilla.

2.  **Construye desde el centro:** El juego exige que utilices la casilla
    central marcada con estrella. Empieza tu primera palabra ahí.
3.  **Crea cruces:** No construyas solo una palabra larga. La mejor forma de
    puntuar es hacer una jugada que forme varias palabras a la vez, por
    ejemplo, colocando una ficha que complete una palabra horizontal y cree una
    nueva palabra vertical al mismo tiempo.
4.  **Usa bien las letras valiosas:** Reserva tus letras de alto valor (como Z,
    Q, J, X) para colocarlas en casillas de bonificación como DL, TL, DW o TW
    y multiplicar su impacto.

<!-- i18n:section:controls -->
### Guía de controles
<!-- i18n:controls:keys -->
- Teclas (alineación)： `Click` `drag` `drag` `click`

*   **Colocar fichas:** Haz clic y arrastra una ficha de letra desde tu atril
    inferior hasta una casilla del tablero.
*   **Recuperar fichas:** Puedes arrastrar una ficha que ya hayas colocado de
    vuelta al atril antes de enviarla si cambias de idea.
*   **Enviar jugada:** Cuando estés conforme con tus palabras, haz clic en el
    botón «Submit Words» para confirmarlas y calcular la puntuación.

<!-- i18n:section:faq -->
### Preguntas frecuentes (FAQ)

<!-- i18n:faq:id=faq:letter-links:q-why-was-my-word-4bce7331 -->
*   **P: ¿Por qué han rechazado mi palabra?**
    *   **R:** Puede ser por varios motivos: 1) No es una palabra válida en el
        diccionario del juego. 2) No todas tus fichas colocadas están conectadas
        en un solo grupo. 3) No has utilizado la casilla central con estrella.

<!-- i18n:faq:id=faq:letter-links:q-how-are-points-calculated-9ad73a75 -->
*   **P: ¿Cómo se calculan los puntos?**
    *   **R:** La puntuación de una palabra es la suma del valor de sus letras,
        multiplicada por los bonus de palabra (DW, TW). Los bonus de letra (DL,
        TL) se aplican primero a letras concretas y luego se calculan los
        multiplicadores de palabra.

<!-- i18n:faq:id=faq:letter-links:q-can-i-play-diagonally-9c4fe9aa -->
*   **P: ¿Puedo formar palabras en diagonal?**
    *   **R:** No. Todas las palabras deben formarse en horizontal (de izquierda
        a derecha) o en vertical (de arriba abajo).


