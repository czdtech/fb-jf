...
1.  **牌区构成：**  
    * **桌面牌区（Tableau）：** 主游戏区域，由七列牌组成。第一列一张、第二列两张……直到第七列七张，每列最上面一张为明牌，其余为暗牌。  
    * **基础牌堆（Foundation）：** 通常位于右上角的四个空位，用于按花色从 A 到 K 依次垒牌。  
    * **牌库：** 剩余未发到桌面的牌，面朝下摆放。  
    * **废牌堆：** 从牌库翻出的明牌会放在这里，供你继续操作。
...
*   **问：如果没有可走的棋怎么办？****
    *   答：** 当桌面和基础堆都没有新移动可做时，就需要从牌库翻牌。如果牌库也抽尽，且仍没有合法移动，则这一局宣告失败。
