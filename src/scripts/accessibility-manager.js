...
export class AccessibilityManager {
  ...
  handleTabNavigation(e) {
    ...
    // 检查是否在焦点陷阱中
    const trapContainer = document.activeElement.closest('[data-focus-trap="true"]');
    if (trapContainer) {
  ...
  handleArrowNavigation(e) {
    const { key, target } = e;
    const container = target.closest('[role="tablist"], [role="menu"], [role="radiogroup"], .audio-player-card');
    
    if (!container) return;
    
    const navigableElements = container.querySelectorAll('[role="tab"], [role="menuitem"], [role="radio"], .audio-play-btn');
  ...
  handleEscape(e) {
    ...
    const openMenus = document.querySelectorAll('.mobile-sheet-content[data-state="open"]');
    
    modals.forEach(modal => {
      if (modal.contains(e.target)) {
        const closeButton = modal.querySelector('[aria-label*="Close"], [aria-label*="关闭"]');
        if (closeButton) {
          closeButton.click();
        }
      }
  ...
  observeModalChanges() {
    ...
    const modals = document.querySelectorAll('[role="dialog"], .mobile-sheet-content');
    modals.forEach(modal => {
      observer.observe(modal, { attributes: true, attributeFilter: ['data-state'] });
    });
  }
  ...
  enhanceInputAccessibility(input) {
    ...
    // 添加错误信息关联
    const errorElement = document.querySelector(`#${input.id}-error, .error[data-for="${input.id}"]`);
  ...
  showKeyboardShortcuts() {
    this.announce('键盘快捷键：Tab键导航，箭头键在组件内导航，回车键或空格键激活，Esc键关闭');
  }
  ...
  checkAccessibility() {
    ...
    const unlabeledInputsWithoutLabels = Array.from(unlabeledInputs).filter(input => !input.labels || input.labels.length === 0);
    if (unlabeledInputsWithoutLabels.length > 0) {
      issues.push(`${unlabeledInputsWithoutLabels.length}个表单字段缺少标签`);
    }
    
    // 检查焦点指示器
    const focusableWithoutIndicator = document.querySelectorAll('button:not(.focus-enhanced), a:not(.focus-enhanced)');
    if (focusableWithoutIndicator.length > 0) {
      issues.push(`${focusableWithoutIndicator.length}个可焦点元素可能缺少焦点指示器`);
    }
