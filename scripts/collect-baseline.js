...
function extractBaseline(htmlContent, filePath) {
  ...
  // Extract canonical
  const canonicalMatch = htmlContent.match(/<link\s+rel="canonical"\s+href="([^"]+)"/);
  result.seo.canonical = canonicalMatch ? canonicalMatch[1] : null;

  // Extract hreflang links
  const hreflangMatches = htmlContent.matchAll(/<link\s+rel="alternate"\s+hreflang="([^"]+)"\s+href="([^"]+)"/g);
  result.seo.hreflang = Array.from(hreflangMatches).map(m => ({
    hreflang: m[1],
    href: m[2]
  })).sort((a, b) => a.hreflang.localeCompare(b.hreflang));

  // Extract JSON-LD
  const jsonLdMatches = htmlContent.matchAll(/<script\s+type="application\/ld\+json">([\s\S]*?)<\/script>/g);
  result.seo.jsonLd = Array.from(jsonLdMatches).map(m => {
    try {
  ...
  if (bodyMatch) {
    ...
    const containerMatches = bodyMatch[1].match(/<div\s+class="([^"]*container[^"]*)"/g);
    result.dom.containers = containerMatches ? containerMatches.slice(0, 5).map(m => m.match(/class="([^"]+)"/)[1]) : [];
...
function collectBaselines() {
  ...
  // Save baseline
  fs.writeFileSync(
    path.join(reportsDir, 'baseline-before.json'),
    JSON.stringify(baseline, null, 2)
  );

  console.log('\n📁 Baseline saved to reports/baseline-before.json');
